...
@main def generateSpecTests(args: String*): Unit = {
  ...
  val parsed = JSON.parse(jsonStr).asInstanceOf[js.Array[js.Dictionary[String]]].toList

  pprintln(parsed)
  // Expect an array of objects
//  val testCases = parsed match {
//    case xs: List[Dictionary[String]] @unchecked => xs
//    case _                                       => sys.error("Expected a JSON array of test case objects")
//  }
//
...
//    testCases.groupBy(tc => tc.get("section").map(_.toString).getOrElse("Uncategorized"))
//
