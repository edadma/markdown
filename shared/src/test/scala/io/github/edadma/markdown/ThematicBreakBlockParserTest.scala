...
class ThematicBreakBlockParserTest extends AnyFunSuite with Matchers {
  ...
  private val tests = Table(
    ...
    ("-*-\n", List(Paragraph(List(Text("-*-"), SoftLineBreak())))),     // contains non-delimiter
    ("--- a\n", List(Paragraph(List(Text("--- a"), SoftLineBreak())))), // extra text
    (" --\n", List(Paragraph(List(Text(" --"), SoftLineBreak())))),     // too few hyphens
    (
      "*_*_*_\n",
      List(Paragraph(List(Emphasis(List(Text("_"))), Emphasis(List(Text("*"))), SoftLineBreak()))),
